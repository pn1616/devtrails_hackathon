...
export default function ClaimsPage({ navigate, worker, policy }) {
  ...
  const handleTrigger = async (trigger) => {
    ...
    // Calculate fraud score via ML
    try {
      const res = await fetch(`${ML}/ml/fraud-score`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          gps_stability: 0.72,
          accelerometer_variance: 0.45,
          network_type: '4g',
          mins_since_last_order: 25,
          filing_speed_seconds: 47,
          cohort_spike: false,
          device_accounts: 1,
          worker_id: worker?.workerId || 'ZPT001'
        })
      });
      ...
      setFraudScore({
        ...
        worker_message: 'Disruption detected in your zone. ₹450 credited to your UPI.',
        signals: FRAUD_SIGNALS_DEMO.map((s, i) => ({
          signal: s.name,
          value: s.safe_value,
          suspicious: false,
          weight: s.weight,
    ...
    try {
      ...
      setClaim({ claimId: `CLM-${Date.now()}`, triggerName: trigger.name, payoutAmount: policy?.coverage || 450 });
    }
  };

  const handleFraudDemo = () => setIsFraud(!isFraud);
  ...
  // Result phase
  const score = fraudScore?.score || 0.18;
  const tier = fraudScore?.tier || 'green';
  ...
  return (
    <div className="page">
      ...
      <div className="score-circle" style={{ borderColor: tierColor }}>
        <div className="score-number" style={{ color: tierColor }}>{score.toFixed(2)}</div>
      ...
      <div className="card-teal" style={{ textAlign: 'center', marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 600, color: tierColor }}>{fraudScore?.auto_action}</div>
        {tier === 'green' && (
      ...
      {tier === 'green' && claim && (
        <div className="card-teal" style={{ textAlign: 'center', marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 12, color: '#22C55E' }}>✓ Credited in under 3 minutes</div>
        </div>
      )}
